...
- 개인정보처리방침/이용약관 실제 문서 작성과 법무 검토 — 랜딩/가입 화면에 국외 이전 고지를
  노출한다는 결정만 확정했다([docs/ADR.md](./docs/ADR.md)의 ADR-015 참고). 문구 확정은 구현
  착수 전 선행 작업.
...
- CRITICAL: 내부 예외 메시지나 스택 트레이스를 사용자에게 그대로 노출하지 않는다 — 항상 이해
  가능한 메시지로 매핑해서 보여준다.
- 업로드 파일 Storage 버킷은 비공개로 설정하고 signed URL(소유자 검증 포함)로만 접근한다
  (ADR-014). 클라이언트가 Storage 경로를 직접 구성해 접근하지 않는다.
