...
- AI 분석 실패(처리 오류, 처리 시간 초과 등 포함) → 분석 실패 안내 → 분석 재시작
...
시작으로 복귀). 구독 상품/결제 정보를 확인하고 Polar(외부 결제 서비스)로 결제를
진행하며, 결제 실패 시 재시도 루프를 거친다. 결제가 완료되면 구독이 활성화되고
대시보드로 복귀해 추가 분석(무료 2회 소진 후 월 2회, 월 최대 4회)을 이용할 수 있다.
구독은 매월 자동 갱신되며, 사용자는 대시보드에서 해지를 요청해 다음 결제 주기부터
갱신을 중단할 수 있다(해지 후에도 현재 결제 주기 종료까지는 혜택 유지).
→ 상세: [F-RPFVZX](./feature-spec.md#f-rpfvzx-분석-이용-현황-및-결제-전환-안내),
[F-UXSBGF](./feature-spec.md#f-uxsbgf-월간-구독-결제-진행),
[F-QVPIEG](./feature-spec.md#f-qvpieg-구독-상태-기반-추가-분석-권한-부여-및-해지)
...
- 결제 관련 정책(구독자 추가 분석 한도, 결제 처리 방식, 구독 갱신/해지)은
  [prd.md의 Product Decisions](./prd.md#product-decisions-확정)에서 확정되었다: 구독자는
  무료 2회 소진 후 추가로 월 2회(월 최대 4회) 분석 가능, 결제는 Polar(외부 결제 서비스)로
  처리, 구독은 매월 자동 갱신되며 사용자가 해지를 요청할 수 있다.
- 소셜 로그인(섹션 1, n7)은 MVP에서 구글만 지원한다.
