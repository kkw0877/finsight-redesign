...
Server Components를 기본으로 쓰고, 업로드 폼·분석 진행 스피너·결제 버튼처럼 인터랙션이
...
**보안 규칙**
- service role 키로 RLS를 우회하는 코드(분석 처리 라우트, 웹훅 핸들러)는 RLS가 없다고 전제하고,
...
2. 추출 + 분석 (동기, ADR-017)
   분석 시작 요청(Client) → POST /api/analysis/start (maxDuration=300s)
   → 무료/구독 잔여 횟수 확인(F-ILHNGA/F-QVPIEG)
   → 기존 processing job이 있으면 지연 확인(updated_at이 300초보다 오래됐으면 failed로 전환
     후 계속 진행, 아니면 409로 거절) — 별도 재수거 cron 없이 이 요청 안에서 처리(ADR-017)
   → analysis_jobs row 생성(status=processing) — user_id 부분 유니크 인덱스로 동시 요청 차단(ADR-013)
   → 같은 요청 안에서 Claude로 거래 내역 추출(ADR-004) → Claude로 소비 분석(ADR-005)
   → analysis_results에 결과 upsert(user_id 기준, ADR-007), analysis_jobs 상태를 completed/failed로 갱신
   → 정상 완료 시에만 무료/구독 사용 횟수 차감 → 응답으로 결과를 바로 반환
   클라이언트는 요청이 끝날 때까지 단일 스피너로 대기 상태를 표시한다(폴링 없음).

3. 결과 조회
   대시보드 로드 시 Client가 GET /api/analysis/latest 호출 → 최신 분석 결과(이력 없음, 최신
   1건만)를 가져와 표시. 새로고침 없이 이미 화면에 있다면 2번 단계의 응답을 그대로 쓴다.

4. 파일 정리
   Supabase pg_cron이 주기 실행 → file_expires_at 지났고 status가 processing이 아닌
   analysis_jobs의 Storage 오브젝트/참조만 삭제(ADR-011, 진행 중인 요청과의 경합 방지)
...
않는다. 아래 5개 테이블만 추가한다. 모두 RLS로 `user_id = auth.uid()` 제한, 분석 처리 라우트와
웹훅 핸들러는 service role 키로 RLS를 우회해 쓴다 — 우회하는 쿼리는 반드시
...
analysis_jobs     (id, user_id, status, source_file_path, source_file_type,
  ...
  status: processing | completed | failed                                          -- ADR-017
  부분 유니크 인덱스: user_id WHERE status = 'processing'                            -- ADR-013
  file_expires_at 기준 pg_cron 삭제 대상(processing 제외)                            -- ADR-011
...
| 라우트 | 메서드 | 인증 | 역할 | 근거 |
|---|---|---|---|---|
| `/api/upload` | POST | 세션(로그인) | CSV/PDF 업로드, 형식/20MB 검증, Storage 저장(job 없이 파일만 저장) | F-ILFWKT |
| `/api/analysis/start` | POST | 세션(로그인) | 잔여 횟수 확인 → 동기로 추출+분석 실행(maxDuration 300s) → 결과를 응답으로 바로 반환(잔여 0이면 402, 이미 진행 중이면 409) | F-PAUYQW, F-ILHNGA, ADR-017 |
...
서버 상태(이용 현황, 구독 상태, 최신 분석 결과)는 API Route 호출로 동기화한다. 분석 요청은
동기 처리(ADR-017)이므로 별도 폴링이 필요 없다 — 요청이 끝나면 응답으로 결과를 바로 받는다.
